...
fun OtpScreen(
    ...
    Text(
        text = if(ticks == 0L) "Resend OTP" else "Resend OTP in ${DateUtils.formatElapsedTime(ticks)} seconds",
        ...
        modifier = Modifier.clickable {
            val options = PhoneAuthOptions.newBuilder(auth)
                ...
                .build()
            Log.d("NumberScreen", "Phone number changed to: +1${phoneNumber}") // Log the phone number change
...
@Composable
fun OtpTextBox(otp: String, flashViewModel: FlashViewModel, modifier: Modifier) {
    BasicTextField(
        value = otp,
        onValueChange = {
        ...
        .addOnCompleteListener(context as Activity) { task ->
            if (task.isSuccessful) {
                ...
                if (task.exception is FirebaseAuthInvalidCredentialsException) {
                    ...
                    Toast.makeText(context, "Invalid OTP entered. Try again!", Toast.LENGTH_SHORT).show()
                }
                // Update UI
            }
        }
